      ...
      <div className="absolute transform-style-3d" style={{ transform: `rotateZ(${p.startAngle}deg)` }}>
        ...
        <div className="absolute transform-style-3d orbit-spin" style={{ animationDuration: `${p.speed}s` }}>
          <div className="absolute transform-style-3d" style={{ transform: `translateX(${p.orbit}px)` }}>
            {/* --- LAYER 1: SPHERE TỰ XOAY (0.55x) --- */}
            <div className="absolute transform-style-3d orbit-anti-spin pointer-events-none" style={{ animationDuration: `${p.speed * 0.55}s` }}>
              <div className="absolute transform-style-3d transition-transform duration-100 ease-out z-10" 
                   style={{ transform: `rotateZ(${-p.startAngle}deg) rotateY(calc(0deg - var(--mouse-x))) rotateX(calc(-75deg - var(--mouse-y)))` }}>

                {/* KHỐI CẦU 3D */}
                <div 
                  className={`absolute transition-all duration-700 ease-out ${isDimmed ? 'opacity-20 scale-90 blur-[4px]' : isHovered ? 'opacity-100 scale-125' : 'opacity-100 scale-100'}`}
                  style={{ width: p.size * 1.8, height: p.size * 1.8, left: -p.size * 0.9, top: -p.size * 0.9 }}
                >
                  <div className="absolute inset-0 rounded-full border-4 border-white/30 opacity-40" style={{ boxShadow: `0 0 60px ${p.color}80` }} />
                  <div className="absolute inset-0 rounded-full bg-gradient-to-br from-white/60 via-transparent to-transparent opacity-30" style={{ transform: 'rotateX(65deg)' }} />
                  <div 
                    className={`w-full h-full rounded-full relative overflow-hidden border border-white/25 transition-all duration-500 ${isHovered ? 'shadow-[0_0_100px_rgba(255,255,255,0.5)]' : 'shadow-2xl'}`}
                    style={{ 
                  ...
                  >
                    ...
                    <div className="absolute inset-0 opacity-35 mix-blend-color-burn" style={{ backgroundImage: LOCAL_NOISE }} />
                  </div>
                </div>
              </div>
            </div>

            {/* --- LAYER 2: ICON & LABEL LUÔN VUÔNG GÓC (1.0x) --- */}
            <div className="absolute transform-style-3d orbit-anti-spin" style={{ animationDuration: `${p.speed}s` }}>
              <div className="absolute transform-style-3d transition-transform duration-100 ease-out pointer-events-auto z-20" 
                   style={{ transform: `rotateZ(${-p.startAngle}deg) rotateY(calc(0deg - var(--mouse-x))) rotateX(calc(-75deg - var(--mouse-y)))` }}>
                
                {/* HITBOX & ICON */}
                <div 
                  className={`absolute cursor-crosshair transition-all duration-700 ease-out ${isDimmed ? 'opacity-20 scale-90 blur-[4px]' : isHovered ? 'opacity-100 scale-125 z-[250]' : 'opacity-100 scale-100'}`}
                  style={{ width: p.size * 1.8, height: p.size * 1.8, left: -p.size * 0.9, top: -p.size * 0.9 }}
                  onClick={(e) => { e.stopPropagation(); playSound('click'); onClick(p); }}
                  onMouseEnter={() => { onHover(p.id); playSound('hover'); }}
                  onMouseLeave={() => onLeave()}
                >
                  <Icon className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[55%] h-[55%] text-white drop-shadow-[0_6px_25px_rgba(0,0,0,0.9)] pointer-events-none transition-transform duration-500 ${isHovered ? 'scale-110' : ''}`} strokeWidth={1.3} />
                  {isHovered && <PlanetParticleBurst color={p.color} active={true} />}
                </div>

                {/* LABEL BILLBOARD */}
                <div className={`absolute flex flex-col items-center transition-all duration-700 ease-out pointer-events-none ${isDimmed ? 'opacity-0 scale-75' : 'opacity-100 scale-100'} ${isHovered ? 'scale-110 z-[260]' : ''}`}
